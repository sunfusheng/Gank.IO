...
public class RefreshableLayout extends LinearLayout {
    ...
    private RefreshableLayoutListener mRefreshViewListener;
    // -- footer view
     ...
     * 被刷新的view
     */
    private RefreshableContentView mContentView;
    ...
    public RefreshableLayout(Context context, AttributeSet attrs) {
        ...
        mContentView = new RefreshableContentView();
        mHolder = new RefreshableHolder();
    ...
    private void initWithContext(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
                    R.styleable.RefreshableLayout, 0, 0);
            try {
                isHeightMatchParent = a.getBoolean(
                        R.styleable.RefreshableLayout_isHeightMatchParent, true);
                isWidthMatchParent = a.getBoolean(
                        R.styleable.RefreshableLayout_isHeightMatchParent, true);
                autoRefresh = a.getBoolean(
                        R.styleable.RefreshableLayout_autoRefresh, false);
                autoLoadMore = a.getBoolean(
                        R.styleable.RefreshableLayout_autoLoadMore, true);
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                a.recycle();
    ...
    private void addHeaderView() {
        if (mHeaderView == null) {
            mHeaderView = new RefreshableLayoutHeader(getContext());
        }
        dealAddHeaderView();
    }
        ...
        if (indexOfChild(mFooterView) == -1) {
            if (needAddFooterView()) {
                ...
                try {
                    ...
                    new RuntimeException("RefreshableLayout is allowed to have one and only one child");
                }
            }
    ...
    private void attachContentView() {
        mContentView.setContentView(RefreshableLayout.this.getChildAt(1));
    ...
    private void addFooterView() {
        if (mFooterView == null) {
            mFooterView = new RefreshableLayoutFooter(getContext());
        }
        dealAddFooterView();
    }
...
//                        paddingLeft + paddingRight, getMeasuredWidth()-lp.leftMargin - lp.rightMargin);
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mHolder.hasHeaderPullDown()) {
                    if (mEnablePullRefresh && !mStopingRefresh && !mPullRefreshing && mHolder.mOffsetY > mHeaderViewHeight) {
                        ...
                        mState = RefreshableLayoutState.STATE_REFRESHING;
    ...
    public RefreshableContentView getContentView() {
        return mContentView;
    }
            ...
            if (mEnablePullRefresh && !mPullRefreshing) {
                if (mHolder.mOffsetY > mHeaderViewHeight) {
                    if (mState != RefreshableLayoutState.STATE_READY) {
                        ...
                        mState = RefreshableLayoutState.STATE_READY;
                    }
                } else {
                    if (mState != RefreshableLayoutState.STATE_NORMAL) {
    ...
    private void updateFooterHeight(int deltaY) {
        if (mEnablePullLoad) {
            if (needAddFooterView()) {
                if (isEmptyViewShowing()) {
                    ...
                    if (mState != RefreshableLayoutState.STATE_LOADING) {
                        mFooterCallBack.onStateRefreshing();
                        mState = RefreshableLayoutState.STATE_LOADING;
                    }
                }
        ...
        if (needAddFooterView() || enableRecyclerViewPullUp) {
            if (mEnablePullUpWhenLoadCompleted || !mContentView.hasLoadCompleted()) {
               ...
               /* if (!needAddFooterView() && mContentView.getState() != RefreshableLayoutState.STATE_COMPLETE && autoLoadMore) {
    ...
    public void stopRefresh(boolean success) {
        if (mPullRefreshing == true) {
            ...
            mState = RefreshableLayoutState.STATE_COMPLETE;
...
//        mFooterCallBack.onStateRefreshing();
    ...
    public void setRefreshableLayoutListener(RefreshableLayoutListener l) {
        mRefreshViewListener = l;
        mContentView.setRefreshableLayoutListener(l);
    }
