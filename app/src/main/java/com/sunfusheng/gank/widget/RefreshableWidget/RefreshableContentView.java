...
import com.sunfusheng.gank.widget.RefreshableWidget.listener.OnBottomLoadMoreTime;
import com.sunfusheng.gank.widget.RefreshableWidget.listener.OnTopRefreshTime;
import com.sunfusheng.gank.widget.RefreshableWidget.recyclerview.BaseRecyclerAdapter;
import com.sunfusheng.gank.widget.RefreshableWidget.recyclerview.RefreshableSpanSizeLookup;
import com.sunfusheng.gank.widget.RefreshableWidget.util.Utils;


public class RefreshableContentView implements OnScrollListener, OnTopRefreshTime, OnBottomLoadMoreTime {
    private View child;
    private int mTotalItemCount;
    private OnTopRefreshTime mTopRefreshTime;
    private OnBottomLoadMoreTime mBottomLoadMoreTime;
    private RefreshableLayout mContainer;
    private OnScrollListener mAbsListViewScrollListener;
    private RecyclerView.OnScrollListener mRecyclerViewScrollListener;
    private RefreshableLayout.RefreshableLayoutListener mRefreshViewListener;
    ...
    private RefreshableLayoutState mState = RefreshableLayoutState.STATE_NORMAL;
    /**
     * 当已无更多数据时候，需把这个变量设为true
     */
    private boolean mHasLoadComplete = false;
    private int mPinnedTime;
    private RefreshableHolder mHolder;
    private RefreshableLayout mParent;

    public void setParent(RefreshableLayout parent) {
        mParent = parent;
    }
    ...
    public void setHolder(RefreshableHolder holder) {
        mHolder = holder;
    }
     ...
     * @param container
     */
    public void setContainer(RefreshableLayout container) {
    ...
    private void setScrollViewScrollListener() {
        if (child instanceof RefreshableScrollView) {
            RefreshableScrollView scrollView = (RefreshableScrollView) child;
            scrollView.setOnScrollListener(mParent, new RefreshableScrollView.OnScrollListener() {
                @Override
...
//                        addFooterView(true);
    ...
    private void setRecyclerViewScrollListener() {
        ...
        if (layoutManager != null && layoutManager instanceof GridLayoutManager) {
            ...
            gridLayoutManager.setSpanSizeLookup(new RefreshableSpanSizeLookup(adapter, gridLayoutManager.getSpanCount()));
        }
        initFooterCallBack(adapter,mParent);
    }

    public void initFooterCallBack(BaseRecyclerAdapter adapter,RefreshableLayout parent) {
        if (!mSilenceLoadMore) {
            if (adapter != null) {
    ...
    private void doAutoLoadMore(BaseRecyclerAdapter adapter, RecyclerView.LayoutManager layoutManager) {
        if (!mIsLoadingMore && isOnRecyclerViewBottom() && mHideFooter) {
            ...
            setState(RefreshableLayoutState.STATE_NORMAL);
        }
    }

    private boolean isFooterEnable() {
        if (mState != RefreshableLayoutState.STATE_COMPLETE && mParent != null && mParent.getPullLoadEnable()) {
            return true;
        }
        return false;
    }
    ...
    public void startLoadMore(boolean silence, BaseRecyclerAdapter adapter, RecyclerView.LayoutManager layoutManager) {
        ...
        if (!hasLoadCompleted()) {
            ...
            setState(RefreshableLayoutState.STATE_LOADING);
        } else {
            loadCompleted();
        }
    }
    ...
    public void releaseToLoadMore(boolean loadmore) {
        ...
        if (loadmore) {
            if (mState != RefreshableLayoutState.STATE_RELEASE_TO_LOADMORE && !addingFooter) {
                mFooterCallBack.onReleaseToLoadMore();
                setState(RefreshableLayoutState.STATE_RELEASE_TO_LOADMORE);
            }
        } else {
            if (mHideFooter) {
                doReadyState();
            } else {
                if (mState != RefreshableLayoutState.STATE_READY) {
                    mFooterCallBack.onStateFinish(false);
                    setState(RefreshableLayoutState.STATE_READY);
                }
            }
        }
    }
    ...
    private void doReadyState() {
        if (mState != RefreshableLayoutState.STATE_READY && !addingFooter) {
            mFooterCallBack.onStateReady();
            setState(RefreshableLayoutState.STATE_READY);
        }
    }
    ...
    public void setLoadComplete(boolean hasComplete) {
        ...
        if (!hasComplete) {
            mState = RefreshableLayoutState.STATE_NORMAL;
        }
...
//        mTotalItemCount = 0;
        ...
        mState = RefreshableLayoutState.STATE_FINISHED;
    }
...
//        return isBottom();
    ...
    public void loadCompleted() {
        ...
        if (mState != RefreshableLayoutState.STATE_COMPLETE) {
            mFooterCallBack.onStateComplete();
            setState(RefreshableLayoutState.STATE_COMPLETE);
    ...
    private void setState(RefreshableLayoutState state) {
        if (mState != RefreshableLayoutState.STATE_COMPLETE) {
            mState = state;
        }
    }

    public RefreshableLayoutState getState() {
        return mState;
    }
...
//        mTotalItemCount = 0;
    ...
    public void setRefreshableLayoutListener(RefreshableLayout.RefreshableLayoutListener refreshViewListener) {
